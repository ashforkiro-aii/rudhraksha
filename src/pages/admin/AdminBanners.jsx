...
const COLORS = [
  { label: "Gold", bg: "from-[#1C1006] to-[#3d1f00]", accent: "#D97706" },
...
const LINK_OPTIONS = [
  ...
  { label: "1 Mukhi", value: "/products?category=1+Mukhi" },
  { label: "2 Mukhi", value: "/products?category=2+Mukhi" },
  { label: "5 Mukhi", value: "/products?category=5+Mukhi" },
...
const inp = "w-full bg-white border border-[#E5D8C8] rounded-lg px-3 py-2 text-sm text-[#1C1006] placeholder-[#8B6A4A] focus:outline-none focus:border-[#5D3A1A]"
  ...
  return (
    <div className={`bg-gradient-to-r ${banner.bg} rounded-xl flex items-center justify-between px-4 py-3 gap-3 min-h-[90px]`}>
      <div className="flex-1 min-w-0">
        ...
        <p className="text-[#1C1006] text-sm font-bold leading-tight">{banner.title || "Title"}</p>
...
function ProductSelector({ form, setForm }) {
  ...
  return (
    <select
      ...
      className="w-full bg-white border border-gray-200 rounded-lg px-3 py-2 text-sm text-[#1C1006] focus:outline-none focus:border-[#5D3A1A]"
    >
...
function BannerForm({ initial, onSave, onCancel }) {
  ...
  return (
    <div className="bg-white border border-[#E5D8C8] rounded-xl p-4 space-y-4">
      <div className="grid grid-cols-2 gap-3">
        ...
        <div className="col-span-2">
          ...
          <div className="flex gap-2 items-start">
            <label className="flex-1 flex items-center gap-2 px-4 py-2.5 border border-dashed border-[#E5D8C8] hover:border-[#5D3A1A]/40 rounded-lg cursor-pointer transition-all bg-[#FAFAFA]">
              <input ref={fileRef} type="file" accept="image/*" className="hidden" onChange={handleImageUpload} />
              {uploading
                ? <><Loader2 size={15} className="text-[#5D3A1A] animate-spin" /><span className="text-gray-400 text-sm">Uploading...</span></>
                : <><ImagePlus size={15} className="text-[#5D3A1A]" /><span className="text-gray-400 text-sm">Upload from device</span></>
              }
            </label>
            {form.image && (
              <div className="relative w-16 h-12 rounded-lg overflow-hidden border border-[#5D3A1A]/20 flex-shrink-0">
      ...
      <div className="flex gap-2">
        ...
        <button type="button" onClick={handleSave} disabled={saving || uploading}
          className="flex-1 py-2 bg-[#5D3A1A] text-white font-semibold rounded-lg text-sm hover:bg-[#7A4E28] disabled:opacity-60 flex items-center justify-center gap-1">
...
function LogoManager() {
  ...
  useEffect(() => {
    getSetting("site_logo_url").then(val => setLogoUrl(val || "")).catch(() => setLogoUrl("")).finally(() => setLoading(false))
  }, [])
  ...
  const handleSave = async () => {
    setSaving(true)
    try { await setSetting("site_logo_url", logoUrl.trim()); toast.success("Logo saved!") }
  ...
  const handleClear = async () => {
    setSaving(true)
    try { await setSetting("site_logo_url", ""); setLogoUrl(""); toast.success("Logo cleared (using default)") }
  ...
  return (
    <div className="bg-white border border-[#E5D8C8] rounded-xl p-5 space-y-4">
      ...
      <div className="flex items-center gap-2 mb-1">
        ...
        <h2 className="text-base font-bold text-[#5D3A1A]" style={{ fontFamily: "Georgia, serif" }}>Site Logo (Navbar)</h2>
      </div>
      ...
      {loading ? <div className="flex items-center gap-2 text-gray-400 text-sm"><Loader2 size={14} className="animate-spin" /> Loading...</div> : (
        <>
          {logoUrl && (
            <div className="flex items-center gap-4 p-3 bg-[#1A0A02] rounded-xl border border-[#3D1F0A]">
              <img src={logoUrl} alt="Logo preview" className="w-16 h-16 rounded-full object-cover border-2 border-[#C8860A]" onError={e => { e.target.style.display = "none" }} />
              <div>
                <p className="text-white text-xs font-semibold">Current Logo</p>
                <p className="text-white/50 text-xs mt-0.5">Shown in navbar top-left</p>
              </div>
            </div>
          )}
          <label className="flex items-center gap-2 px-4 py-2.5 border border-dashed border-[#E5D8C8] hover:border-[#5D3A1A]/40 rounded-lg cursor-pointer transition-all bg-[#FAFAFA] w-fit">
            ...
            {uploading
              ...
              : <><ImagePlus size={15} className="text-[#5D3A1A]" /><span className="text-gray-500 text-sm">Upload logo (will open crop tool)</span></>
            }
          </label>
          <div>
            <label className={lbl}>Or paste image URL</label>
            <input value={logoUrl} onChange={e => setLogoUrl(e.target.value)} placeholder="https://..." className={inp} />
          </div>
...
function HeroBgManager() {
  ...
  useEffect(() => {
    getSetting("hero_bg_image_url").then(val => setImageUrl(val || "")).catch(() => setImageUrl("")).finally(() => setLoading(false))
  }, [])
  ...
  const handleCropped = async (blob) => {
    ...
    setUploading(true)
    try {
      const path = `hero/hero_bg_${Date.now()}.jpg`
      const { error } = await supabase.storage.from("product-images").upload(path, blob, { contentType: "image/jpeg", upsert: true })
      if (error) throw error
      const { data } = supabase.storage.from("product-images").getPublicUrl(path)
      setImageUrl(data.publicUrl)
      toast.success("Image cropped & uploaded! Click Save to apply.")
    } catch (err) { toast.error(err.message || "Upload failed") }
    finally { setUploading(false) }
  }
  ...
  const handleSave = async () => {
    setSaving(true)
    try { await setSetting("hero_bg_image_url", imageUrl.trim()); toast.success("Hero background image saved!") }
    catch (err) { toast.error(err.message || "Failed to save") }
    finally { setSaving(false) }
  }
  ...
  const handleClear = async () => {
    setSaving(true)
    try { await setSetting("hero_bg_image_url", ""); setImageUrl(""); toast.success("Hero background cleared") }
    catch (err) { toast.error(err.message || "Failed to clear") }
    finally { setSaving(false) }
  }
      ...
      {loading ? <div className="flex items-center gap-2 text-gray-400 text-sm"><Loader2 size={14} className="animate-spin" /> Loading...</div> : (
        <>
          ...
          <label className="flex items-center gap-2 px-4 py-2.5 border border-dashed border-[#E5D8C8] hover:border-[#5D3A1A]/40 rounded-lg cursor-pointer transition-all bg-[#FAFAFA] w-fit">
            ...
            {uploading
              ? <><Loader2 size={15} className="text-[#5D3A1A] animate-spin" /><span className="text-gray-500 text-sm">Uploading...</span></>
              : <><ImagePlus size={15} className="text-[#5D3A1A]" /><span className="text-gray-500 text-sm">Upload & Crop image</span></>
            }
          </label>
...
export default function AdminBanners() {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        ...
        {!showForm && !editBanner && (
          <button onClick={() => setShowForm(true)}
            className="flex items-center gap-2 px-4 py-2 bg-[#5D3A1A] text-white font-semibold rounded-lg hover:bg-[#7A4E28] transition-all text-sm">
          ...
          {banners.map((banner, idx) => (
            <div key={banner.id}>
              {editBanner?.id === banner.id ? (
                ...
                <div className="bg-white border border-[#E5D8C8] rounded-xl overflow-hidden shadow-sm">
                  <BannerPreview banner={banner} />
                  <div className="flex items-center justify-between px-4 py-2 border-t border-[#5D3A1A]/10">
                    <div className="flex items-center gap-1">
                      <span className="text-gray-500 text-xs">#{idx + 1}</span>
                      <button onClick={() => move(idx, -1)} disabled={idx === 0} className="p-1 text-gray-500 hover:text-[#1C1006] disabled:opacity-30"><ChevronUp size={14} /></button>
                      <button onClick={() => move(idx, 1)} disabled={idx === banners.length - 1} className="p-1 text-gray-500 hover:text-[#1C1006] disabled:opacity-30"><ChevronDown size={14} /></button>
                    </div>
                    <div className="flex items-center gap-2">
                      <button onClick={() => setEditBanner(banner)} disabled={saving} className="flex items-center gap-1 px-3 py-1.5 text-xs text-gray-400 hover:text-[#5D3A1A] border border-[#5D3A1A]/20 rounded-lg disabled:opacity-40"><Edit2 size={12} /> Edit</button>
      ...
      {saving && (
        <div className="fixed bottom-4 right-4 bg-[#5D3A1A] text-white px-4 py-2 rounded-lg text-sm font-medium flex items-center gap-2 shadow-lg">

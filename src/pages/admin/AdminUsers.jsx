  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-[#5D3A1A]" style={{ fontFamily: "Georgia, serif" }}>Users</h1>
        ...
        <button onClick={fetchAllUsers} disabled={loadingUsers}
          className="flex items-center gap-2 px-3 py-2 bg-[#5D3A1A] text-white text-xs font-medium rounded-lg hover:bg-[#7A4E28] disabled:opacity-60 transition-all">
          <RefreshCw size={13} className={loadingUsers ? "animate-spin" : ""} /> Refresh
        </button>
      </div>

      <div className="grid grid-cols-3 gap-4">
        <div className="bg-white border border-gray-200 rounded-xl p-5"><Users size={18} className="text-[#5D3A1A] mb-3" /><p className="text-2xl font-bold text-[#5D3A1A]">{allUsers.length}</p><p className="text-gray-500 text-xs mt-1">Total Customers</p></div>
        <div className="bg-white border border-gray-200 rounded-xl p-5"><DollarSign size={18} className="text-green-400 mb-3" /><p className="text-2xl font-bold text-[#5D3A1A]">{formatINR(totalRevenue)}</p><p className="text-gray-500 text-xs mt-1">Total Revenue</p></div>
        <div className="bg-white border border-gray-200 rounded-xl p-5"><ShoppingBag size={18} className="text-blue-400 mb-3" /><p className="text-2xl font-bold text-[#5D3A1A]">{formatINR(avgOrderValue)}</p><p className="text-gray-500 text-xs mt-1">Avg Order Value</p></div>
      </div>
      ...
      <div className="space-y-3">
        ...
        : users.map(u => {
          ...
          const tierColor = tier === "Gold" ? "text-[#5D3A1A] bg-[#5D3A1A]/10" : tier === "Silver" ? "text-gray-600 bg-gray-500/10" : "text-orange-400 bg-orange-500/10"
            ...
            <div key={u.userId} className="bg-white border border-gray-200 rounded-xl overflow-hidden">
              <div className="flex items-center justify-between p-4 cursor-pointer hover:bg-gray-50 transition-colors"
                ...
                <div className="flex items-center gap-3">
                  <div className="w-9 h-9 bg-[#5D3A1A]/15 rounded-full flex items-center justify-center flex-shrink-0">
                    <span className="text-[#5D3A1A] text-sm font-bold">{u.name[0]?.toUpperCase()}</span>
                  </div>
                  <div>
                    <p className="text-[#1C1006] text-sm font-medium">{u.name}</p>
                ...
                <div className="flex items-center gap-3">
                  ...
                  {loadingDetails[u.userId]
                    ? <div className="w-4 h-4 border-2 border-[#5D3A1A] border-t-transparent rounded-full animate-spin" />

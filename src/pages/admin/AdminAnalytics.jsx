...
const COLORS = ['#5D3A1A', '#7A4E28', '#3D5A99', '#4E6FA8', '#1A3A6B', '#3B2310', '#5B7DB1', '#6B8EC4']
...
export default function AdminAnalytics() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-[#5D3A1A]" style={{ fontFamily: 'Georgia, serif' }}>Analytics</h1>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { icon: DollarSign, label: 'Total Revenue', value: formatINR(stats.totalRevenue), color: 'text-[#5D3A1A]' },
          ...
          <div key={i} className="bg-white border border-gray-200 rounded-xl p-5">
            ...
            <p className="text-2xl font-bold text-[#5D3A1A]">{k.value}</p>
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-5">
        <h3 className="text-[#5D3A1A] font-medium mb-4">Revenue Trend (Last 14 Days)</h3>
        <ResponsiveContainer width="100%" height={260}>
          <AreaChart data={stats.last14Days}>
            <defs>
              <linearGradient id="navyGrad" x1="0" y1="0" x2="0" y2="1">
                <stop offset="5%" stopColor="#5D3A1A" stopOpacity={0.3} />
                <stop offset="95%" stopColor="#5D3A1A" stopOpacity={0} />
              </linearGradient>
            </defs>
            ...
            <Area type="monotone" dataKey="revenue" stroke="#5D3A1A" fill="url(#navyGrad)" strokeWidth={2} name="Revenue" />
          </AreaChart>
        </ResponsiveContainer>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Weekly Orders */}
        <div className="bg-white border border-gray-200 rounded-xl p-5">
          <h3 className="text-[#5D3A1A] font-medium mb-4">Weekly Orders</h3>
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={weeklyData}>
              ...
              <Bar dataKey="orders" fill="#5D3A1A" radius={[4, 4, 0, 0]} name="Orders" />
            </BarChart>
          </ResponsiveContainer>
        </div>
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-5">
          <h3 className="text-[#5D3A1A] font-medium mb-4">Revenue by Category</h3>
          <ResponsiveContainer width="100%" height={220}>
            <PieChart>
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-5">
        <h3 className="text-[#5D3A1A] font-medium mb-4 flex items-center gap-2">
          <MapPin size={16} className="text-[#5D3A1A]" /> Location Analytics — Top Cities
        </h3>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <ResponsiveContainer width="100%" height={240}>
            <BarChart data={stats.cityData} layout="vertical">
              ...
              <Tooltip {...TooltipStyle} />
              <Bar dataKey="count" fill="#7A4E28" radius={[0, 4, 4, 0]} name="Orders" />
            </BarChart>
          </ResponsiveContainer>
          ...
          <div className="space-y-3">
            {stats.cityData.map((c, i) => (
              <div key={c.city} className="flex items-center gap-3">
                <span className="text-[#5D3A1A] text-xs w-5 font-bold">{i + 1}</span>
                <div className="flex-1">
                  <div className="flex justify-between mb-1">
                    <span className="text-[#1C1006] text-xs">{c.city}</span>
      ...
      {/* Top Products Bar */}
      <div className="bg-white border border-gray-200 rounded-xl p-5">
        <h3 className="text-[#5D3A1A] font-medium mb-4">Best Selling Products</h3>
        <ResponsiveContainer width="100%" height={240}>
